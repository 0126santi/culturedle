...
async function getQuestionOfTheDay() {
    ...
    try {
        ...
        // Log controlado para evitar ruido en producción
        console.log('Gemini OK - bytes:', text.length);
        const match = text.match(/\{[\s\S]*\}/);
        if (match) {
            try {
                const obj = JSON.parse(match[0]);
                if (obj.question && obj.answer && /^[a-zA-Z0-9 ]+$/.test(normalize(obj.answer))) {
                    // recortar pregunta si vino más larga de lo pedido
                    const q = String(obj.question).trim();
                    const shortQ = q.split(/\s+/).slice(0, 20).join(' ');
                    return { question: shortQ, answer: normalize(obj.answer) };
                }
            } catch (e) {
        ...
        console.error('Error al llamar a Gemini:', e?.message || e);
